...
class DataPrecessForSentence(Dataset):
    ...
    def get_input(self, file):
        df = pd.read_csv(file,sep='/t',header=None,encoding='utf-8')
        sentences_1 = list(map(HanziConv.toSimplified, df.iloc[:, 0].values))
        sentences_2 = list(map(HanziConv.toSimplified, df.iloc[:, 1].values))
